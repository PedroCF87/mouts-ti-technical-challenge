...
describe('UsersController (e2e)', () => {
  ...
  const expectUserShape = (user: any) => {
    ...
    expect(user).not.toHaveProperty('password'); // password nunca deve ser retornada
  ...
  it('should create a valid user (POST /users)', async () => {
    ...
    expect(response.body.role).toBe('user'); // padrão esperado
    expect(response.body.isActive).toBe(true); // padrão esperado
